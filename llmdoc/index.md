...
> Watt：构建在 Cloudflare 上的 Agent Infra 平台，由持续开发 loop 按 DOD Phase 0~7 增量实现（Phase 0/1/2/3 已关门，当前 Phase 4 Tool Layer + Agent Runtime）。规格真源在 `Docs/`，本目录是其检索层与过程记忆。启动阅读顺序见 [startup.md](startup.md)（此处不重复）。
