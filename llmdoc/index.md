...
> Watt：构建在 Cloudflare 上的 Agent Infra 平台，由持续开发 loop 按 DOD Phase 0~7 增量实现（Phase 0/1/2/3/4 已关门，Phase 5 Task + Scheduler 三项 DoD 全勾待关门）。规格真源在 `Docs/`，本目录是其检索层与过程记忆。启动阅读顺序见 [startup.md](startup.md)（此处不重复）。
