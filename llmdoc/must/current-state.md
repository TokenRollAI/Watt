...
- **R33 可用性冲刺新增面**（六期并行 + 线上实测增补，详见 PROGRESS Round 33 与 4 个新决策记录）：
  ...
  - CLI 改名 **`@token-roll/watt`**（tsup bundle dist/bin.js + `release:cli`；**npm 未真发布**）+ 新命令族 `watt init`（TUI 九步向导）/ `watt setup feishu`（幂等五步）/ `watt secret`（三命令，值走 stdin）。
