...
> 本文档随轮次更新。最后更新：2026-07-03（Round 18，Phase 4 关门）。

## 阶段

- 规格真源：`Docs/{Vision,Architecture,Proto,Plugin,Reference}.md` + `DOD.md`（验收）+ `LOOP.md`（执行契约）。
- Phase 0~3 已关门（Round 3/7/10/13）；**Phase 4（Tool Layer + Agent Runtime）已关门**（2026-07-03 Round 18：质量关口 4 维 review + 对抗核查确认 2 BLOCKER + 10 MAJOR 全修（0 误报），DoD 线上复验通过（tool call 新契约 + agent spawn/send/expect-schema/terminate 真实模型全链），证据 `PROGRESS.md` Round 14~18）。
- **下一目标：Phase 5（Task + Scheduler）项 1**——**先派 investigator 调研**：Workflows 适配、Scheduler DO、`deep-research` 与 `auto-delivery-lite` 两个部署模板。
...
## 源码现状（Round 18 后，测试共 751 个：shared 6 + core 314 + cli 93 + gateway 338+1skip（@llm 门控）；core 覆盖率 100%，门禁挂 verify）
...
- `packages/gateway`（Hono Worker，入口 export fetch/queue + 四个 DO 类）：
  ...
  - migrations：`migrations/0001`（watt-policies）、`migrations-events/0001`（watt-events）、`migrations-context/0001`（watt-context）、**`migrations-providers/0001~0003`**（tool_registry / agent_registry / model_providers，watt-providers 库）。
- `packages/cli`（包名 `watt-cli`）— **十一命令族**：`status` / `login` / `whoami` / `policy` / `audit` / `event tail|get|subs` / `channel list|set` / `context ls|cat|put|patch|mount|unmount` / **`tool mount|ls|describe|call`** / **`agent`（六命令：def write/spawn/send/tree/terminate 等）** / **`provider`（三命令，含 set-default）**。token 顺序 `WATT_TOKEN` env > `~/.watt/credentials.json`（0600）；未认证分层 exit 2/1。
...
| 类型 | 资源 |
|---|---|
| D1 ×5 | `watt-policies`（0001） / `watt-providers`（**0001~0003**：tool/agent/model registry，Round 15/17） / `watt-audit` / `watt-events`（0001） / `watt-context`（0001） |
...
| DO ×4 | `EVENT_ROUTER` / `CONTEXT_REGISTRY` / **`AGENT_INSTANCE`（AgentInstance，Round 17）** / **`AGENT_CORRELATION`（AgentCorrelation，Round 17/18）** |
