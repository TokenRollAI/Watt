...
> 本文档随轮次更新。最后更新：2026-07-03（Round 10，Phase 2 关门）。

## 阶段

- 规格真源：`Docs/{Vision,Architecture,Proto,Plugin,Reference}.md` + `DOD.md`（验收）+ `LOOP.md`（执行契约）。
- Phase 0 已关门（2026-07-02 Round 3）；Phase 1 已关门（2026-07-02 Round 7）；**Phase 2（Event Gateway）已关门**（2026-07-03 Round 10：质量关口 4 维 review + 对抗核查确认 15 MAJOR 全修，DoD 线上重跑全通，证据 `PROGRESS.md` Round 9/10）。
- **下一目标：Phase 3（Context Layer）项 1**（四动词语义单测）。
- Phase 2 遗留：17 条 MINOR backlog（dedupe check-then-put 并发窗、fetchDeliverer 无超时、~help #21、cursor 分页 #22 等）不阻塞，随后续 Phase 顺手修；Phase 2 实现声明簇见 doc-gaps #25（source.kind 规约 / HITL 系统投递免 Check / Signal 桩留 Phase 5 / DLQ 重放留 Phase 6 等到期收口项）。
...
## 源码现状（Round 10 后，测试共 354 个：shared 6 + core 163 + cli 41 + gateway 144；core 覆盖率 100%（247/247 语句、166/166 分支），门禁挂 verify）
...
- `packages/gateway`（Hono Worker，入口 `export default {fetch, queue}` + export EventRouter）：
  ...
  - `src/http/` — `auth.ts` / `errors.ts` / `oauth.ts`；`routes.ts`（JWKS 公开 + `/htbp/platform/{whoami,policy,audit}` + `/htbp/platform/event`（List/Get/Publish/Subscribe/Unsubscribe/ListSubscriptions；Publish 入参 eventInputSchema 校验 400 + **无条件规约 `source.kind='webhook'`**，doc-gap #25①）+ `/htbp/platform/channel` 四动词）；`inbound.ts`（真实化 `/channels/:id/inbound`：无认证验签即认证；**bodyRaw 字节精确**——arrayBuffer 严格 UTF-8 解码、失败转 base64；404 未知 channel / 403 disabled·错签 / 非 webhook adapter 显式 501）。
  - `src/index.ts` — notFound/onError 兜底 + 剩余规范树 501 占位（platform/{agent,task,scheduler}、tools、context），注册在认证中间件之前（501 优先于 401）。
  - `migrations/0001_auth_core.sql`（watt-policies 库）+ `migrations-events/0001_event_gateway.sql`（events 表 + 5 索引 + channels 表，watt-events 库）。
- `packages/cli`（包名 `watt-cli`，非 @watt/cli）— 七命令族：`status` / `login`（device flow，`--json` 输出 NDJSON）/ `whoami` / `policy list|add|rm` / `audit list` / `event tail|get|subs`（tail = 轮询 List + occurredAt 游标 + `--once`；显式 limit 200 + 游标**含端重查 + 同毫秒 id 去重**（Set 仅存游标毫秒防无界）+ 满页 stderr 截断警告）/ `channel list|set`。token 读取顺序 `WATT_TOKEN` env > `~/.watt/credentials.json`（0600）；未认证分层：本地无 token exit 2 / 服务端 401 exit 1。
...
- **watt-gateway 已部署**（含 DO EventRouter + Queue consumer 双绑定），双 URL：
...
- `wrangler deploy` 本身不跑 D1 migrations，但 **`pnpm deploy:all` 已内置两库 migrations**：`d1 migrations apply watt-policies --remote` + `d1 migrations apply watt-events --remote`（均幂等）；再有新库带 migrations 时仍须同步扩展。另有根 `pnpm migrate` 独立脚本。
...
| 类型 | 资源 |
|---|---|
| D1 ×4 | `watt-policies`（0001 migration） / `watt-providers` / `watt-audit` / `watt-events`（0001 migration，Round 9） |
