...
> - `release.yml` — 推 `v*` tag（或手动 workflow_dispatch）时构建部署产物并发布 `@tokenroll/watt` 到 npm。
>
> 发布认证二选一（release.yml 两者兼容）：
> 1. **Trusted Publishing（推荐，免 token）**：npmjs.com → 包 `@tokenroll/watt` → Settings → Trusted Publisher，
...
> 2. **NPM_TOKEN secret**：npmjs.com 创建 Granular Access Token（Read and write，Packages and scopes 选 @tokenroll），
