...
async function handleRequest(request: Request, env: AppEnv): Promise<Response> {
  ...
  try {
    ...
    // WATT VENDOR PATCH: watt-toolbridge is a headless proxy target (the Watt
    // gateway only ever forwards /htbp/* here); it ships no UI, so the ASSETS
    // binding is absent. Serve static assets only when a host provides ASSETS,
    // otherwise 404. This is the sole deviation from upstream index.ts.
    if (env.ASSETS) {
      return env.ASSETS.fetch(request);
    }
    return errorResponse(404, 'not_found', 'No asset serving on this deployment.');
  } catch (error) {
    return errorResponse(500, 'internal_error', messageOf(error));
  }
}
